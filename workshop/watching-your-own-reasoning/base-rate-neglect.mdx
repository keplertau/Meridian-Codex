...
<text x="360" y="35" textAnchor="middle" fill="#D0D0D0" fontSize="14" fontWeight="700" letterSpacing="0.05em">99% Sensitivity and Specificity for a Rare Condition</text>
...
<text x="660" y="298" textAnchor="end" fill="#8A8A8A" fontSize="11" fontStyle="italic" opacity="0.7">Sensitivity and specificity: 99%</text>
<text x="660" y="316" textAnchor="end" fill="#8A8A8A" fontSize="11" fontStyle="italic" opacity="0.7">Intuition: "I probably have it"</text>
</svg>
</div>

The medical-diagnostic application of the math is one of the most consequential. Suppose a condition has a prevalence of 0.1%, and a screening test has 99% sensitivity and 99% specificity. In 10,000 people, the test will find about ten true positives and produce about one hundred false positives. A positive result therefore corresponds to roughly a 9% chance of actually having the condition, not 99%. Sensitivity and specificity describe how the test behaves given disease status; neither is the probability of disease given a positive result. That probability also depends on the base rate. The bias has actual costs in actual medicine. Patients who interpret a single positive screening test as a confirmed diagnosis make decisions on a model of their situation that does not survive contact with the math.
