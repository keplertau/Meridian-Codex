...
**Why this tool is in the Workshop.** Bayesian Reasoning is here because calibration needs an inference structure, not only post-hoc scoring. Calibration Training checks confidence against outcomes after the fact; Bayesian Reasoning trains the present-tense question: what was already likely, how diagnostic is this evidence, and how far should confidence move? Base Rate Neglect names one failure this structure corrects, but the Bayesian frame supplies the positive discipline. It belongs here because evidence-proportional confidence needs a way to weigh new information before the outcome resolves.
