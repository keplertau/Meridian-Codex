...
This category is being published as a v0.1 scaffold. The category belongs in the Workshop now, but its dedicated tool set has not cleared protocol yet. That should be visible rather than hidden.
...
Future tools should enter slowly through the source-inherited candidate protocol. The likely territory is rich enough that moving too quickly would be a mistake: Buddhist mindfulness and equanimity practices, Stoic training around impressions and disturbance, somatic awareness and nervous-system regulation lineages, breath practices, contemplative traditions that train presence under discomfort, and clinical approaches that distinguish regulation from avoidance. The inclusion bar is distinct contribution. A tool belongs here only if it helps a practitioner remain present under pressure in a way the existing Foundation tools do not already cover. Until at least one dedicated tool clears that bar, this page should be read as an architectural marker and candidate map, not as the completed toolkit for the category.
...
The boundary with [Holding Beliefs Without Identity](/workshop/holding-beliefs-without-identity) is important because the same lived moment can activate both categories.
