...
The resource does not have to be grassland. It can be a fishery, groundwater basin, atmosphere, public trust, road capacity, team attention, open-source maintainer energy, democratic legitimacy, or an information environment. The resource has two defining properties: it is hard to exclude people from using it, and one person's use can reduce what remains available for others. Economists call this a common-pool resource.
...
Without institutional analysis, the tool can collapse into a slogan: commons fail, therefore privatize or command. Ostrom's work makes the reading more exact. A commons has a governance problem. The outcome depends on boundaries, monitoring, trust, sanction, local knowledge, conflict resolution, and legitimacy.
...
**Within the category.** [Prisoner's Dilemma](/inheritance/reading-whats-operating/prisoners-dilemma) reads the strategic temptation to defect; Tragedy of the Commons reads one resource form of that temptation. [Moloch](/inheritance/reading-whats-operating/moloch) reads the broader competitive race that can drive commons depletion. [Rules-in-Use](/inheritance/reading-whats-operating/rules-in-use) asks what access, extraction, monitoring, and sanction rules actually operate. [Entropy](/inheritance/reading-whats-operating/entropy) keeps the maintenance cost visible: maintenance is permanent, and unpaid maintenance becomes decay.

**Across the Inheritance.** Mechanism Design becomes relevant when incentives need redesign. Bond's Stewardship Across Time and Belonging Through Practice become relevant when the shared resource depends on identity, obligation, inheritance, and repeated cooperation rather than contract alone.
