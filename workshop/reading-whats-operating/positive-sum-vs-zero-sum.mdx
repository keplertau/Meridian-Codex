...
aiSummary: "Positive-Sum vs Zero-Sum Framing is Reality tool inside Reading What's Operating. It diagnoses the payoff shape people believe they are inside: zero-sum when one side's gain is treated as another side's loss, positive-sum when cooperation or exchange can increase total value, and negative-sum when conflict or defection destroys value for all sides. Mechanism: payoff field, perceived scarcity, fixed-pie assumption, gains from trade, specialization, trust, repeated interaction, institutions, and framing effects. Practice: identify the resource, test whether the pie is fixed, find hidden joint gains, check whether trust and rules can unlock them, and name cases where zero-sum conflict is real. Lineage: game theory's zero-sum and variable-sum distinction, von Neumann and Morgenstern, bargaining theory, gains from trade, comparative advantage, repeated games, and Robert Wright's nonzero-sum historical synthesis. Cross-references: Prisoner's Dilemma, Nash Equilibrium, Mechanism Design, Schelling Points, Tragedy of the Commons, and Bond cooperation tools."
discipline: reality
