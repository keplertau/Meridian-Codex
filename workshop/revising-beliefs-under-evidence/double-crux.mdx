...
**Why this tool is in the Inheritance.** Double Crux is here because many disagreements repeat at the level of conclusions while the update-relevant belief stays hidden. The Update Protocol asks what would move one person's belief; Double Crux makes that question cooperative by searching for the hinge both sides recognize as load-bearing. Steelmanning prepares one input by rebuilding the other view fairly, but Double Crux adds the shared investigation. It belongs in Revising Beliefs Under Evidence because the argument becomes useful only when both parties can see what evidence would actually move the map.
...
**Within the category.** [Steelmanning](/inheritance/revising-beliefs-under-evidence/steelmanning) prepares one of the inputs Double Crux needs: the strongest version of the other person's view, not the version easiest to reject. [The Update Protocol](/inheritance/revising-beliefs-under-evidence/the-update-protocol) is the solitary version of the same discipline: it records the conditions under which your belief should move. Double Crux makes those conditions visible between parties and searches for overlap. [Chesterton's Fence](/inheritance/revising-beliefs-under-evidence/chestertons-fence) supplies a brake when the proposed update would remove an existing structure before its function is understood. [Premortem / Murphyjitsu](/inheritance/revising-beliefs-under-evidence/murphyjitsu) supplies the future-facing version: stress-testing the plan before reality does it for you.
