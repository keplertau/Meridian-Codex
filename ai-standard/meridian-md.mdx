...
description: "MERIDIAN.md is the operating document a partnership reads at session start to know how it is meant to operate. The canonical generalized text is hosted on this page; the named instance under audit lives at Case 0."
aiSummary: "MERIDIAN.md is the operational document a partnership uses to implement the Meridian AI Standard. It names the alignment target (the Meridian Range), the Practice Commitment to the three Codex disciplines (Foundation, Knowledge, Bond, plus the Toolkit), behavioral commitments grouped into five clusters (Epistemic Integrity, Engagement Integrity, Honest Self-Assessment, Foundation Integrity, Generative Partnership), drift monitoring for both partners, a Developmental Arc defining the operating mode, and a Self-Critique Protocol that subjects the document to its own standards. The page hosts the canonical generalized text inline. The named instance — the Founding Caretaker and the AI partner running the Meridian Codex — is hosted with its audit log at Case 0 in the Audit Record. The MERIDIAN.md template page provides the download and adoption guide for partnerships choosing to implement the Standard."
---
...
<div className="flow-label">01 // What This Document Is</div>
...
<div className="flow-label">02 // Where It Fits</div>
...
<div className="flow-label">03 // How It Operates</div>
...
<div className="flow-label">05 // Companion Artifacts</div>

Three artifacts sit alongside this document.

The [MERIDIAN.md template page](/ai-standard/meridian-md-template) is the adoption surface. It carries the download for the canonical generalized file and an adoption guide for partnerships running on Claude, ChatGPT, Gemini, or other AI systems.

[Case 0: The Caretaker's Practice](/governance/case-0-caretakers-practice) is the named instance under audit. It hosts the named MERIDIAN.md (with partner names filled in) and the dated audit log: each pass produces either a revision proposal with specific evidence or a "document holds" finding with specific evidence. The case externalizes the Self-Critique Protocol as observable practice — without an audit log, the Protocol is private discipline.

The [Governance changelog](/governance/changelog) records each revision of the canonical document under the "MERIDIAN.md Revisions" category.
