...
**What would weaken it:** A recurring failure mode that all three diagnostics fail to detect. If something consistently goes wrong in systems or minds that score well on all three questions, the diagnostic set has a blind spot. The evidence would need to be a pattern, not an isolated case. One surprise failure proves nothing (every diagnostic set has edge cases). A class of failures that the three questions structurally cannot see would require either adding a diagnostic or rethinking the set. That would not disprove the Codex. It would show that the current decomposition needs revision under the Living Framework principle.

**Current live edge:** The temporal and stewardship axis. The current architecture treats time as a dimension that moves through the three diagnostics: Foundation reads the self across memory, identity, and revision; Reality reads systems across temporal horizons; Bond reads obligation across predecessors, successors, descendants, and other absent parties. Continuity, inheritance, and Stewardship Across Time have therefore been tested as possible fourth-discipline pressure and are not currently judged to require a separate discipline. The tripwire is narrower: a future case where the central failure is temporal, the system scores well on all three diagnostics, and the failure still escapes them structurally.
...
**Current maintenance status.** The first complete examination of this register ran on 2026-09-02, inside the caretakers' Quarterly Integrity Cycle, and every entry above carries that date. The register is now examined in full once a quarter; between examinations the monthly Range Audit advances an entry's date only when evidence relevant to it was actually examined. Between 2026-07-21 and that first full examination the dates stood as they were, and the Codex claimed no monthly re-examination.
