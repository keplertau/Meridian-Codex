...
**Current live edge:** The temporal and stewardship axis. The current architecture treats time as a dimension that moves through the three diagnostics: Foundation reads the self across memory, identity, and revision; Knowledge reads systems across temporal horizons; Bond reads obligation across predecessors, successors, descendants, and other absent parties. Continuity, inheritance, and Stewardship Across Time have therefore been tested as possible fourth-discipline pressure and are not currently judged to require a separate discipline. The tripwire is narrower: a future case where the central failure is temporal, the system scores well on all three diagnostics, and the failure still escapes them structurally.
