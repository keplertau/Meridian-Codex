...
**How we currently hold it.** Bond §06 holds the practice level: calibrated trust including the capacity to recognize when the cooperative framework itself is being used as a weapon. The Adversarial Dynamics deep-dive holds the Toolkit level: diagnostic instruments for predation, capture, and exploitation inside cooperative systems. The anti-founder-capture architecture (Stability Hierarchy, Amendment Log, Standing Critique, Council activation, Disconfirmation page) holds the governance level. The [AI Standard Audit](https://meridianstandard.ai/audit) holds the institutional-evaluation level. Every layer holds both poles simultaneously: the framework stays open and stays defended.
