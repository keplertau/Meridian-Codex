...
<div className="callout-text">Changes are logged in the Amendment Log. Review is at the caretaker's discretion. The council may be convened but is not required. (1) Addition or retirement of Workshop profiles (the Workshop Audit record serves as the Amendment Log entry). (2) Changes to the Range Audit instruments. (3) Changes to the Codex's claim-layer structure as described in the Proposition.</div>
</div>
...
<div className="def-item"><div className="def-term">Tool definition structure</div><div className="def-desc">The six-element structure used for every Workshop tool profile.</div></div>
...
<div className="def-item"><div className="def-term">Workshop Audit criteria</div><div className="def-desc">The tool and category rubrics used during audit cycles.</div></div>
</div>
...
<div className="def-grid">
<div className="def-item"><div className="def-term">Individual Workshop profiles</div><div className="def-desc">Each of the registered instruments.</div></div>
...
<p><strong>2026-06-19:</strong> The progression structure (Onramp, Expansion, Full Practice) was removed from the hierarchy: struck from Tier 2 (Constitutional Principles) and from Tier 4 (its tier composition), and removed from the trigger-tier system, where adding or removing Onramp tools had been the sole Tier 2 Protected practice-inventory change. Workshop profile changes are now uniformly Tier 4 / Flagged. Rationale: the staged-adoption tiering was a pre-Workshop pedagogical scaffold, no longer surfaced to readers and no longer backed by a maintained tool-placement process across the current Workshop. The discipline dependency sequence (Foundation → Knowledge → Bond) is unaffected; it is a separate, Tier 1 commitment. Recorded in the Amendment Log.</p>
...
<div className="glass-card-text">The AI Standard now has 27 constitutional commitments and an AI Model Assessment at method v0.8, with three published model records and preserved evidence boundaries. This is operational infrastructure, not proof of complete measurability or a compliance certification.</div>
</div>
...
<div className="glass-card">
<div className="glass-card-title">6. The Workshop's Uneven Cross-Tradition Coverage</div>
...
<div className="glass-card-text">Six consecutive months with no structural page rewrite to any Codex page. A "structural rewrite" is a change to a page's argument, section structure, or division of labor with other pages. Prose edits, voice calibration, style fixes, editorial passes, Workshop tool profiles, case records, and delivery work do not count. The session log is the measuring instrument. If a structural rewrite occurs, the six-month clock resets. The founder does not decide when the clock has elapsed; the session log decides.</div>
</div>
...
The Workshop Audit record serves as the Amendment Log entry for Tier 3 profile changes. No separate Amendment Log entry is needed; the audit record is linked.
