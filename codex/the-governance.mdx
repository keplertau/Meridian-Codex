...
**Tier 1: Hard Constraints.** These define what the Codex is. They are the structural commitments without which the framework would no longer be recognizable as itself. Revision is theoretically possible but requires evidence so extraordinary that it would amount to building a successor framework rather than evolving this one. Tier 1 contains: the Hard Constraint itself, the Prime Directive, the Meridian Range as the foundational concept, Control and Decay as coupled failures, the three-discipline structure, and the partnership principle.

**Tier 2: Constitutional Principles.** These organize how the Codex works. They are core commitments that shape the framework's logic, its governance, and its relationship to practitioners. Revisable, but only under strict conditions: full Council deliberation, compelling evidence that the principle is failing the Range, and demonstration that the proposed replacement strengthens the architecture. Tier 2 contains: the Meridian Compact, the Non-Ownership Clause, the Reciprocity Principle, the three diagnostic questions, the living framework principle, the governance phase structure, the Meridian Council's existence and mandate, the Open Source principle, the discipline-level failure modes, and the progression structure.
