# Contributing to the Meridian Codex

The Codex is a living framework, and substantive contributions from anywhere in the world are part of how it will evolve. The impulse is welcome. The formal architecture to receive those contributions is still being built.

## What GitHub Is (and Is Not)

This repository exists as a convenient way to read, copy, download, and adapt the Codex under CC BY 4.0. It is the **distribution surface**, not the operational one.

The operational home of the Codex is **[meridiancodex.com](https://meridiancodex.com)**. That is the single source of truth for what the framework is, how it evolves, how it is governed, and, in time, how it accepts contributions from anywhere in the world.

The Meridian AI Standard is a peer public project at **[meridianstandard.ai](https://meridianstandard.ai)**, with its own repository at **[keplertau/Meridian-AI-Standard](https://github.com/keplertau/Meridian-AI-Standard)**. Standard-specific issues, adoption questions, Range Locator records, and Case Record proposals belong with that project, not in this repository.

## The Contribution Architecture Is In Development

The Codex is in its **founding period**. During this phase, structural changes are held by the Founding Caretaker while the framework's core architecture stabilizes. The formal contribution process — how proposals are submitted, evaluated, debated, and integrated beyond the founding period — is part of the governance layer still being built.

When it lands, it will be published in the governance section of [meridiancodex.com](https://meridiancodex.com/governance).

## Until Then

**For small, specific fixes** — typos, broken links, factual errors, internal inconsistencies — you can open a GitHub Issue. Keep it specific: what you found, where it appears, and how it affects the text or its use.

**For sustained objection to the framework itself**, the **[Standing Critique](https://meridiancodex.com/governance/standing-critique)** is the appropriate destination. The Standing Critique publishes substantive objections alongside the Codex's honest responses. A developed argument that the framework is wrong about something belongs there, not in a GitHub Issue.

**For proposed new tools, practices, or lineage additions**, do not open a broad "please add this" issue. The Workshop admits tools through source-lineage and functional-fit review, not through representational coverage or enthusiasm. A useful issue names the specific gap the proposed tool fills, the source lineage it comes from, and why the existing Workshop does not already cover that work.

**For anything else**, the best thing you can do is practice the Codex, read it critically, and talk about it publicly. The full contribution infrastructure will arrive when the framework is stable enough to hold it. When it does, the criteria will be the same disciplines the Codex teaches: honest inquiry, steelmanning, calibrated claims, good faith, and service to the Meridian Range.

## What Belongs In This Repository

This repository is intentionally narrow. It contains only public Codex content and a small set of repository information files.

Do not submit private working files, drafts, site-code changes, brand assets, generated media, local workflow notes, or project-memory records here. The live website code is maintained separately. This repository's `.gitignore` uses a whitelist model so non-public material is ignored by default.

## Privacy And Public Record

GitHub Issues, pull requests, comments, and commits are public. Do not include secrets, private correspondence, private personal information, API keys, internal documents, or anything you are not willing to publish.

See [PRIVACY.md](PRIVACY.md) for the repository-specific privacy note.

---

*The Codex is built for all sentient life. Its development should reflect that, without pretending every surface is ready to receive every kind of contribution yet.*
