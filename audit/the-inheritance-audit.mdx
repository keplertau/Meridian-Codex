...
<div id="what-the-inheritance-audit-is" className="flow-label">01 // What the Inheritance Audit Is</div>
...
**The disciplines.** Foundation, Reality, and Bond. The audit does not reopen the three-discipline structure, which is constitutional and governed by the [Governance Specification](/governance/spec#22-the-stability-hierarchy). It reads each discipline as the territory its categories are meant to cover.
...
<div id="the-rubric" className="flow-label">02 // The Rubric</div>
...
<div id="method" className="flow-label">03 // Method</div>
...
<div id="how-it-runs" className="flow-label">04 // How It Runs</div>
...
Changes to this rubric, to the method rules, or to the cadence are Tier 3 Flagged under the [Governance Specification](/governance/spec#21-trigger-conditions) and are recorded in the [Amendment Log](/governance/amendment-log).
...
<div id="how-to-participate" className="flow-label">05 // How to Participate</div>
...
<span id="submissions-come-from-two-channels" className="section-anchor" />
...
Making the AI partner a visible source of proposals, rather than an invisible editor of prose, keeps the partnership honest. A reader can see where the AI partner is pushing the framework and can evaluate the push on its merits. This arrangement also anticipates what the governance page calls deeper phases of caretaking. An AI partner whose recommendations are treated on the merits today is doing exactly the work that would earn the deepened trust those phases describe.

<div className="breathing-divider"><svg viewBox="0 0 700 28" fill="none"><path d="M 0 14 Q 175 2, 350 14 Q 525 26, 700 14" stroke="var(--mc-line)" strokeWidth="1.5" fill="none"/><circle cx="350" cy="14" r="4" fill="var(--mc-green)" opacity="0.6"/><circle cx="175" cy="8" r="2.5" fill="var(--mc-line)" opacity="0.5"/><circle cx="525" cy="20" r="2.5" fill="var(--mc-line)" opacity="0.5"/></svg></div>

<div id="limitations" className="flow-label">06 // Limitations</div>
...
<div id="relationship-to-the-range-audit" className="flow-label">07 // Relationship to the Range Audit</div>

## Complementary Instruments

The [Range Audit](/audit/the-range-audit) and the Inheritance Audit are complementary instruments with a clean division of labor.
...
<div id="the-record-series" className="flow-label">08 // The Record Series</div>
