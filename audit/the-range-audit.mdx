...
<div id="what-the-range-audit-is" className="flow-label">What the Range Audit Is</div>
...
The instrument is built from the Codex's own architecture. The three disciplines provide the evaluative lens. The Inheritance provides the diagnostic probes. The Prime Directive provides the orientation: does this system serve or undermine the conditions for cooperation across minds?

The method itself practices the three disciplines. The steelman comes first. The evaluation connects every domain to the integrated system. The Compact test checks whether the system practices identity-as-practice or identity-as-fortress. The Prime Directive connection asks whether the system's existence expands or contracts the conditions for flourishing.
...
<div id="how-it-works" className="flow-label">How It Works</div>
...
- Inheritance probes: which specific profiles from the complete current Inheritance inventory were applied, and what they revealed

**What this requires of the auditor:** The Inheritance probes are the Range Audit's mechanism for transparency. They make the auditor's reasoning visible. When a domain finding cites a specific Inheritance profile (Calibration Training, Mechanism Design, Cult Dynamics, etc.), the reader can see which diagnostic lens produced the finding. This is how the Range Audit keeps its own process open to examination.

### Step 3: Integrate Through the Three Disciplines

After evaluating the six domains, the auditor integrates the findings through the three disciplines as a unified lens. This is not a summary. It is a second-order analysis that asks: what do the domain findings reveal when read through the Foundation (epistemic integrity), Reality (structural coherence), and the Bond (cooperative capacity) as an integrated system?
...
<div id="what-it-does-not-do" className="flow-label">What It Does Not Do</div>
...
**It depends on the auditor's judgment.** The six-step method provides structure, but within each step, the auditor makes interpretive choices. Two auditors applying the same method to the same system may produce different findings. The Inheritance probes exist to make these choices visible, but they do not eliminate subjectivity. The honest response to this is not to pretend objectivity. It is to make the reasoning transparent enough that others can evaluate the auditor's work.

**It is v0.1.** The instrument will evolve. As it is applied to more systems and as the findings are tested against reality, the method will be refined. Steps may be added. Domains may be revised. The Inheritance probes may become more structured. The living framework principle applies to the Range Audit itself.
...
<div id="the-monthly-codex-audit" className="flow-label">The Monthly Codex Audit</div>
...
The monthly audit follows the full six-step method. It reads the complete Codex text at the time of evaluation. It produces findings, Range positions, Inheritance probes, the Compact test, the Prime Directive connection, and open questions. Previous open questions are the first items checked.

A prior audit does not substitute for current corpus coverage. If a later method permits unchanged material to carry forward from a verified baseline, that change must be made formally and must define the baseline, source-delta check, material reread, and resulting claim ceiling. Until such a method exists, a delta-based or targeted reading is a declared protocol deviation.
...
<div id="applying-the-range-audit" className="flow-label">Applying the Range Audit</div>
